...
function executePlan(planFilename: string, options?: ExecutionOptions): void {
  ...
  // Cost warning: each step is a manual Copilot session consuming premium requests
...
async function executeSwarm(
  ...
  options?: {
    ...
    session?: string; costEstimateOnly?: boolean; maxPremiumRequests?: number;
  }
): Promise<void> {
  ...
  // Always print a brief cost summary so users know what to expect
  ...
  if (options?.costEstimateOnly) {
    // Detailed breakdown for --cost-estimate-only
  ...
  try {
    ...
    if (options?.costEstimateOnly) swarmOptions.costEstimateOnly = true;
    if (options?.maxPremiumRequests !== undefined) swarmOptions.maxPremiumRequests = options.maxPremiumRequests;
    ...
    // Cost summary (visible after dashboard clears)
    const actualRequests = completed + failed + context.results.filter(r => (r as any).retryCount > 0).length;
...
async function main(): Promise<void> {
  ...
  if (command === 'quick') {
    ...
    // Cost warning before any Copilot invocation
    ...
    const quickEstimate = quickCostEstimator.estimate(
      { goal: task, createdAt: new Date().toISOString(), steps: [{ stepNumber: 1, agentName: flags.agent || 'backend_master', task, dependencies: [], expectedOutputs: [] }] },
  ...
  if (command === 'plan') {
    ...
    const useInnerFleetEnabled = args.includes('--useInnerFleet') || args.includes('--wrap-fleet');
    const costEstimateOnly = args.includes('--cost-estimate-only');
    const maxPremiumIdx = args.indexOf('--max-premium-requests');
    const maxPremiumRequests = maxPremiumIdx !== -1 && args[maxPremiumIdx + 1]
      ? parseInt(args[maxPremiumIdx + 1], 10)
      : undefined;
    ...
    try {
      const options: {
        ...
        costEstimateOnly?: boolean; maxPremiumRequests?: number;
      } = {};
      ...
      if (costEstimateOnly) options.costEstimateOnly = true;
      if (maxPremiumRequests !== undefined && !isNaN(maxPremiumRequests)) options.maxPremiumRequests = maxPremiumRequests;
    ...
    if (args.includes('--useInnerFleet') || args.includes('--wrap-fleet')) runOpts.useInnerFleet = true;
    ...
    if (args.includes('--cost-estimate-only')) runOpts.costEstimateOnly = true;
    ...
    if (maxPremIdx !== -1 && args[maxPremIdx + 1]) {
      ...
      if (!isNaN(parsed)) runOpts.maxPremiumRequests = parsed;
    }
