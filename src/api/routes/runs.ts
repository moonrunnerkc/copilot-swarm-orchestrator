...
export function createRunsRouter(runsDir: string): Router {
  ...
  router.get('/:runId', (req: Request, res: Response) => {
    ...
    // Load cost attribution if available
    const costPath = path.join(runPath, 'cost-attribution.json');
    if (fs.existsSync(costPath)) {
      try { result.costAttribution = JSON.parse(fs.readFileSync(costPath, 'utf8')); } catch { /* skip */ }
    }
